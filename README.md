...
Endpoint GET /api/client/servers/{server}/files/folder-size chỉ cho root administrator gọi. Panel ưu tiên helper chạy trên host volume; nếu helper không thành công, controller dùng API Wings để duyệt thư mục, bỏ qua symlink và giới hạn số thư mục con.
